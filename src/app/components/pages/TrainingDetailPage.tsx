...
export function TrainingDetailPage() {
  ...
  const nextStart = hasStarts ? training.scheduledStarts!.find(s => s.status !== 'full') : null;
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-24 lg:pb-12">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 pb-14">
          <div className="max-w-3xl">
            <div className="flex flex-wrap items-center gap-2 mb-4">
              <span className="inline-block bg-white/20 backdrop-blur-sm px-3 py-1 rounded-full text-sm text-white border border-white/20">{category.name}</span>
              {isCustom && (
                <span className="inline-flex items-center gap-1.5 bg-violet-500/30 backdrop-blur-sm border border-violet-300/40 px-3 py-1 rounded-full text-sm text-violet-100">
                  <Briefcase className="w-3.5 h-3.5" />Skräddarsydd
                </span>
              )}
              {isScheduled && (
                <span className="inline-flex items-center gap-1.5 bg-emerald-500/30 backdrop-blur-sm border border-emerald-300/40 px-3 py-1 rounded-full text-sm text-emerald-100">
                  <CalendarDays className="w-3.5 h-3.5" />Schemalagd
                </span>
              )}
              {training.isPopular && (
                <span className="inline-flex items-center gap-1.5 bg-amber-400/30 backdrop-blur-sm border border-amber-300/40 px-3 py-1 rounded-full text-sm text-amber-100">
                  <Flame className="w-3.5 h-3.5" />Populär
                </span>
              )}
            </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="lg:hidden flex gap-3">
              ...
              {hasStarts && nextStart && (
                <button onClick={() => handleApply(nextStart.id)} className="flex-1 flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-3 rounded-xl font-medium text-sm transition-colors shadow-sm">
                  <CalendarDays className="w-4 h-4" />Anmäl dig
                </button>
              )}
            </div>
            ...
            {hasStarts && (
              <div className="flex gap-1 bg-slate-100 p-1 rounded-xl">
                <button onClick={() => setActiveTab('overview')} className={`flex-1 py-2 px-4 rounded-lg text-sm font-medium transition-all ${activeTab === 'overview' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900'}`}>
                  Om utbildningen
                </button>
              ...
              <>
                ...
                <section className="bg-white rounded-xl border border-slate-200 p-6 md:p-8">
                  ...
                  <div className="grid md:grid-cols-2 gap-6 pt-6 border-t border-slate-100">
                    <div>
                      <h3 className="font-semibold text-slate-900 mb-2 flex items-center gap-2"><Users className="w-4 h-4 text-slate-400" />Målgrupp</h3>
                      <p className="text-slate-600 text-sm leading-relaxed">{training.targetAudience}</p>
                    </div>
                    <div>
                      <h3 className="font-semibold text-slate-900 mb-2 flex items-center gap-2"><MapPin className="w-4 h-4 text-slate-400" />Format</h3>
            ...
            {activeTab === 'starts' && hasStarts && (
              <section className="bg-white rounded-xl border border-slate-200 p-6 md:p-8">
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h2 className="text-xl font-bold text-slate-900">Planerade utbildningsstarter</h2>
                    <p className="text-sm text-slate-500 mt-1">{training.scheduledStarts!.length} tillfälle{training.scheduledStarts!.length !== 1 ? 'n' : ''} tillgängliga</p>
                  </div>
                  {training.scheduledStarts!.some(s => s.status === 'few_spots') && (
                    <span className="hidden sm:flex items-center gap-2 text-sm text-amber-700 bg-amber-50 border border-amber-200 px-3 py-1.5 rounded-lg">
                      <Star className="w-4 h-4" />Begränsat antal platser
                    </span>
                  )}
                </div>
          ...
          <div className="hidden lg:block lg:col-span-1">
            <div className="sticky top-24 space-y-4">
              ...
              {hasStarts && (
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                  ...
                  <div className="p-5">
                    {nextStart ? (
                      <>
                        <div className="mb-3"><StatusBadge status={nextStart.status} size="sm" /></div>
                        <div className="bg-slate-50 rounded-lg p-3 mb-4 space-y-2 text-sm">
                          <div className="flex justify-between">
                            <span className="text-slate-500">Nästa start</span>
                            <span className="font-semibold text-slate-900">{new Date(nextStart.startDate).toLocaleDateString('sv-SE', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
                          </div>
                          <div className="flex justify-between">
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className={`font-semibold ${nextStart.availableSpots <= 5 ? 'text-amber-600' : 'text-emerald-600'}`}>{nextStart.availableSpots} st</span>
                          </div>
                        </div>
                        <button onClick={() => handleApply(nextStart.id)} className="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-3 rounded-lg font-medium text-sm transition-colors mb-2">
                          <CalendarDays className="w-4 h-4" />Anmäl dig nu
                        </button>
                        <button onClick={() => setActiveTab('starts')} className="w-full text-center px-4 py-2.5 border border-slate-200 rounded-lg text-slate-700 hover:bg-slate-50 transition-colors font-medium text-sm">
                      ...
                      <p className="text-sm text-slate-600 text-center py-2">Inga lediga platser just nu.</p>
                    )}
                  </div>
                </div>
              )}
              ...
              <div className="bg-slate-50 rounded-xl border border-slate-200 p-5">
                ...
                <dl className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <dd className="font-medium text-slate-900">{training.trainingType === 'both' ? 'Skräddarsydd & Schemalagd' : training.trainingType === 'custom' ? 'Skräddarsydd' : 'Schemalagd'}</dd>
                  </div>
                </dl>
              </div>
              ...
              <div className="bg-white rounded-xl border border-slate-200 p-5">
                ...
                <button onClick={handleShare} className="w-full flex items-center justify-center gap-2 border border-slate-200 text-slate-700 hover:bg-slate-50 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors">
                  <Share2 className="w-4 h-4" />Kopiera länk
                </button>
              </div>
            </div>
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 lg:hidden bg-white border-t border-slate-200 px-4 py-3 flex gap-3 z-40 shadow-lg">
        ...
        {hasStarts && nextStart && (
          <button onClick={() => handleApply(nextStart.id)} className="flex-1 flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white py-3 rounded-xl font-medium text-sm transition-colors">
            <CalendarDays className="w-4 h-4" />Anmäl dig
          </button>
        )}
      </div>
