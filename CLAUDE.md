...
- [ ] **E-postnotifieringar (v1) väntar på miljövariabler + Resend-konto** (byggt 2026-08-07): kod, databaswebhooks och cron är klara och pushade, men skickar inga riktiga mejl förrän `RESEND_API_KEY`, `ADMIN_NOTIFICATION_EMAIL`, `NOTIFY_WEBHOOK_SECRET` och `CRON_SECRET` är satta i Vercel (Preview och senare Production), och ett Resend-konto med verifierad avsändardomän finns. Tills dess loggas misslyckade skickningsförsök som `status='failed'` i `email_log` (admin-flik) — bevisar att kedjan fungerar, men inga mejl når fram.
