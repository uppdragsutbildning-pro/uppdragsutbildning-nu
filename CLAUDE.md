...
- [ ] **Paket C temar bara den delade headern, inte hela sidan** (2026-09-03): `Root.tsx` visar partnerns logga/tagline/primärfärg och exponerar `--marketplace-primary`/`--marketplace-secondary` som CSS-variabler, men knappar, nav-länkar, hero-sektioner m.m. i övriga delade komponenter använder ännu hårdkodade Tailwind-färger (blue-600/green-600), inte variablerna. Färdigställs naturligt i takt med Paket D (storefront-query) när den faktiska temade kurslistningen byggs.
