...
- [ ] **E-postnotifieringar (v1) väntar på miljövariabler + Brevo-konto** (byggt 2026-08-07, bytt från Resend till Brevo 2026-08-07 av GDPR/EU-dataresidens-skäl – Resend är amerikanskt): kod, databaswebhooks och cron är klara och pushade, men skickar inga riktiga mejl förrän `BREVO_API_KEY`, `ADMIN_NOTIFICATION_EMAIL`, `NOTIFY_WEBHOOK_SECRET` och `CRON_SECRET` är satta i Vercel (Preview och senare Production), och ett Brevo-konto med verifierad avsändardomän (`uppdragsutbildning.com`) finns. Tills dess loggas misslyckade skickningsförsök som `status='failed'` i `email_log` (admin-flik) — bevisar att kedjan fungerar, men inga mejl når fram.
