...
- [ ] **Kanonisk kurs-URL använder `/training/{uuid}`, inte specens `/kurs/{course-slug}`** (2026-09-03, Paket H): specen (avsnitt 11) föreslår en SEO-vänlig slug-baserad URL-struktur, men befintlig routing använder redan `/training/:id` med UUID överallt i kodbasen. Att införa slugs skulle kräva ett nytt fält på `trainings`, redirects och bredare routing-ändringar – bedömdes som en separat, större SEO-förbättring utanför denna specs scope. Kanonisk-taggen pekar därför på `uppdragsutbildning.nu/training/{uuid}`.
