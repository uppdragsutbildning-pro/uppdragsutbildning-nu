...
- [ ] **E-postnotifieringar (v1) – Preview/staging klart verifierat, Production kvarstår** (byggt 2026-08-07, Resend → Brevo 2026-08-07 av GDPR/EU-dataresidens-skäl, Brevo-domän `uppdragsutbildning.com` verifierad samma dag): webhook- och cron-flödet är fullt end-to-end-testat i staging (`{"ok":true}`/`{"escalated":N}`, riktiga mejl mottagna, loggade som `status='sent'` i `email_log`). Kvarstår innan produktionslansering: sätt `BREVO_API_KEY`, `ADMIN_NOTIFICATION_EMAIL`, `NOTIFY_WEBHOOK_SECRET`, `CRON_SECRET` och `APP_URL` för Production i Vercel (egna värden, inte samma som Preview), kör migrationerna (`pg_net`-trigger, `escalated_at`, `email_log`, bypass-header) mot production-Supabase, och sätt production-vault-secrets (`notify_webhook_url`, `notify_webhook_secret`, `vercel_bypass_secret`).
